...
- **Warning**: Warning, the service may be malfunctioning or a problem is about to occur, and early intervention is recommended to prevent escalation.
- **Critical**: Severe, the service is experiencing widespread issues or has been interrupted, affecting users, and immediate intervention is required.
...
- **Event Severity**: Alert events from different integration sources (such as Zabbix and Nightingale) have different severity enumeration values. Flashduty will map these values to the above three standard severity levels according to certain rules. For specific mapping relationships, please refer to the documentation for the specific integration.
- **Alert Severity**: Equal to the highest severity level in the associated event.
- **Incident Severity**: Equal to the highest severity level in the associated alert.

### Processing Progress

- **Pending**: After an incident is triggered, the processing progress defaults to "Pending." The system will initiate automatic dispatch, assign handlers, and notify them.
...
> [!TIP]
> Automatic recovery of an incident will cause (processing progress) to be automatically closed; however, manually closing an incident will have no impact on its status.
...
**Automatic Triggering**: When Flashduty receives an alert event (such as a Zabbix notification) reported by an integration, the event automatically triggers an alert, which then triggers an incident.

**Manual Triggering**: Manually click the **Create Incident** button in the Flashduty console, fill in the title, description, severity, and other information to trigger a new incident.
...
You can set different dispatch strategies for different time periods or types of incidents to achieve flexible dispatch. The system allows you to set multiple dispatch stages under one dispatch strategy. If the assigned personnel of the current stage do not complete the incident confirmation and processing within the specified time, the system will automatically upgrade to the next stage. Upgrading is equivalent to reassigning the incident to other personnel.
...
> [!TIP]
> Note that **an incident will only be notified after it is dispatched**. Without dispatch, there will be no notifications.

If you assign an incident to a duty schedule where no one is on call (vacant), the system will not send notifications to any individuals. However, if you configure a group chat channel, messages will still be pushed to the group chat.

**3. Claim and Resolve**

After on-call personnel receive the notification, they can immediately claim the incident. You can claim the incident in **voice calls** and **instant messages**. After claiming, the incident processing progress will change to **In Progress**.
...
> [!TIP]
> Flashduty supports dedicated integration and shared integration models. Deliver the alert to the exclusive integration under the collaboration space, and the incident will be triggered under this collaboration space.
...
> [!TIP]
> Please ensure that you will actively close the alerts, or set the incident to automatically close after a timeout in the collaboration space. When there are too many incidents, console retrieval performance will drop sharply. At that time, the system may close historical incidents without any notification.
...
| Assigned Personnel | No | Incident Handler<br>If not selected, the handler will be matched and assigned based on the collaboration space to which they belong.<br>Otherwise, notifications will be sent directly according to personal preferences |