---
brief: Synchronize Grafana alert events to Kuaimao Nebula via webhook to achieve automatic noise reduction processing of alert events.
---
...
## In Grafana
Grafana versions V4 to V8 have the Legacy Alerting feature enabled by default, while versions V9 and later have the Grafana Alerting feature enabled by default. For function differentiation and activation methods, please refer to the [official documentation](https://grafana.com/docs/grafana/v8.4/alerting/unified-alerting/opt-in/#opt-in-to-grafana-alerting). Please integrate according to the version you have deployed. The steps are as follows.
...
The system sequentially extracts `severity`, `priority`, and `level` from the alert event tags. The corresponding value will be used as the Prometheus alert level. If none are extracted, the system automatically sets the Prometheus alert level to `warning`.
