...
In the FlashDuty incident details, incident tags are displayed in the key:value format, showing various source data information of the alert. These tags originate from various alert events connected to FlashDuty. **When the system-generated tags based on source data do not meet business needs, you can use tag enhancements to enrich the alert with more tag information**, thereby improving incident handling efficiency.
...
- **Incident List:** The incident list now supports quick filtering by tags, allowing users to view and manage all incidents with the same tags centrally, enhancing the efficiency and relevance of incident handling
...
- **Alert Aggregation:** By intelligently using the same or different tags as dimensions in the aggregation process, we can effectively categorize and reduce noise in alert events, improving the clarity and handling efficiency of incidents
- **Silence Rules:** Tags can be included in the silence condition configuration as a basis for judgment, accurately filtering and silencing alerts associated with specific tags, thus achieving advanced customization and noise control in alert management
...
Each alert event in the integration center has a tag enhancement configuration option, which can be configured after the alert event is created
...
- **Restrictions:** When performing tag enhancements, you can configure precondition rules to restrict whether corresponding tags are generated or deleted (off by default). When there are restrictions, if the conditions are not met, the tags will not be generated or deleted. You can go to [Configure Filter Conditions](/conf/how_to_filter) to learn how to configure restrictions.
...
### 2. **Tag Combination**
**Scenario:** The company's log platform can directly access log details through a combination of domain name, event ID, and timestamp. However, the alert information only contains event ID and timestamp tags. Therefore, you need to combine this information into an access URL.
...
### 3. **Tag Mapping**
**Scenario:** When the tag values in the source alert information are not fixed and their meanings are not intuitive, you can map the source tags to new defined tags and values. For example, if the source alert only contains resource type ID information, but you want to reflect the corresponding resource type name for each ID, you can achieve this through mapping.
...
> [!NOTE]
> If you do not want the source tag to still exist, you can achieve this through the deletion rule.